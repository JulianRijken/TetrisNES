...
constexpr Uint32 MAX_ELAPSED_TIME{ 100 }; // limit in case of breakpoint
constexpr Uint32 MAX_FRAME_RATE{ 50 }; // limit in case of breakpoint
...
void Run()
{
	...
	while (!quit)
	{
		...
		if (!quit)
		{
			...
			{
				elapsedSeconds = MAX_ELAPSED_TIME;
			}
			...
			const float frameTime = std::chrono::duration<float>(t3 - t2).count();;

			// Add delay
			constexpr float minFrameTime{ 1.0f / MAX_FRAME_RATE };
			const float sleepTime{std::max(minFrameTime - frameTime, 0.0f) };\
